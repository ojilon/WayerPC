...
def get_file_metadata(file_path: str) -> dict | None:
    ...
    # C: Convert st_mtime using localtime() then format via strftime()
    dt_local = datetime.datetime.fromtimestamp(stat_info.st_mtime, tz=None)
    formatted_date = dt_local.strftime("%m/%d/%Y %I:%M %p")

    return {
        "file_name": path.name,            # C: Extract from path string / dirent.d_name
        "file_type": path.suffix,          # C: Extract extension from path string
...
def copy_path(source_path: str, destination_path: str):
    ...
    if src.is_file():
        dst.parent.mkdir(parents=True, exist_ok=True)
        # C: Standard file copy reads blocks of data into
